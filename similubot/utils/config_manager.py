...
class ConfigManager:
    ...
    def get_max_cache_files(self) -> int:
        """
        Get the maximum number of cached files to keep.

        Returns:
            The maximum number of cached files
        """
        return self.get('download.max_cache_files', 20)

    def get_hash_algorithm(self) -> str:
        """
        Get the hash algorithm for filenames.

        Returns:
            The hash algorithm name (md5, sha1, sha256)
        """
        return self.get('download.hash_algorithm', 'md5')
